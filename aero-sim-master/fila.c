...
void inserir (bool no_ar, fila_ordenada_t *fila, aviao_t *dado) {
	...
	if (fila->n_elementos == 0) { // se a fila estiver vazia, o elemento adicionado será o primeiro elemento e o ultimo elemento
		...
		fila->ultimo = elemento;
   		...
   		fila->ultimo = elemento;
    		...
    		for (size_t i = 0; i < fila->n_elementos; i++) {
      			if (fila[i]->combustivel > combustivel_aviao_a_inserir) {
        			elemento_t *de_maior_combustivel = fila[i];
        			elemento->proximo = de_maior_combustivel;
      				elemento->anterior = de_maior_combustivel->anterior;
        			de_maior_combustivel->anterior->proximo = elemento;
        			de_maior_combustivel->anterior = elemento;
      			}
    		}
  	}
}
